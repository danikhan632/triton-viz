...
def get_visualization_data():
    ...
    visualization_data = {}
    raw_tensor_data = {}
    kernel_src = ""  # Initialize kernel_src
    
    for grid_idx, program_records in records.items():
        print(grid_idx)
        viz_data, raw_data, current_kernel_src = prepare_visualization_data(program_records, tensor_table)
        
        # Convert grid_idx tuple to a string key
        key = f"{grid_idx[0]}_{grid_idx[1]}_{grid_idx[2]}"
        
        visualization_data[key] = viz_data  # Use the new string key
        raw_tensor_data.update(raw_data)
        
        # Store the kernel source code (assuming it's the same for all records)
        if not kernel_src and current_kernel_src:
            kernel_src = current_kernel_src
